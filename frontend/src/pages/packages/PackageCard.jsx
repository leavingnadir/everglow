...
export default function PackageCard({ pkg, onEdit, onDelete, isAdmin }) {
  ...
  const vendorLabel = (value) =>
    VENDOR_CATEGORIES.find((v) => v.value === value)?.label ||
    String(value)
      .replaceAll("_", " ")
      .toLowerCase()
      .replace(/\b\w/g, (c) => c.toUpperCase());

  const includedServices = Array.isArray(pkg?.includedVendors)
    ? pkg.includedVendors.filter(Boolean)
    : [];
  ...
  return (
    <div
      ...
      onMouseEnter={(e) => {
        e.currentTarget.style.borderColor = tierAccent;
    ...
    >
      ...
      {includedServices.length > 0 && (
        <div style={{ marginTop: "12px", marginBottom: isAdmin ? "20px" : "0" }}>
          <p
            style={{
              fontFamily: "var(--font-body)",
              fontSize: "11px",
      ...
      {isAdmin && (
        ...
        >
          <button
            ...
            onMouseLeave={(e) => {
              e.target.style.borderColor = "#D0B8B4";
